...
Bisher wurden alle Werte und Vektoren in verschiedenen Variablen gespeichert. Listen in R bieten die Möglichkeit mehrere objekte in einer Variablen zusammenzufassen. Sie werden mit der `list()` Funktion erstellt:

```{r}
# Zwei vektoren in der gleichen Variablen durch eine Liste:
vec1 = c(1,3,5,7)
vec2 = c("zwei", "vier", "sechs")
numbers = list(vec1, vec2)
numbers
```


- Erstelle eine Liste mit 1. den original Temperaturwerten von oben und 2. gerundeten Temperaturwerten.
- Sieh dir dazu die Hilfeseite zu `round` an. Runde die Temperaturwerte auf eine Nachkommastelle.
